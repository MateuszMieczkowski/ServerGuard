...
import { updateDashboard, Dashboard, Graph } from "../api/dashboards-service";
import {
...
export const UpdateDashboardDialog: React.FC<UpdateDashboardDialogProps> = ({
  resourceGroupId,
  agentId,
  open,
  setOpen,
  ...
  const [graphs, setGraphs] = useState<Graph[]>(dashboard.graphs);
  ...
  const handleUpdateDashboard = async () => {
    ...
    try {
      await updateDashboard(resourceGroupId, agentId, dashboardId, {
        name: dashboardName,
        graphs,
      });
      onUpdate();
      ...
      showErrorSnackbar("Failed to yodate dashboard.");
      setIsSubmitting(false);
      return;
    }
  };
  ...
  return (
    <>
      ...
      >
        <DialogTitle width={200}>Update Dashboard</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            fullWidth
