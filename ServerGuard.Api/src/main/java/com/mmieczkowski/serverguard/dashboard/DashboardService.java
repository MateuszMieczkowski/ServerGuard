...
public class DashboardService {
    ...
    @Transactional
    public CreateDashboardResponse createDashboard(UUID resourceGroupId, UUID agentId, CreateDashboardRequest request) {
        ...
        List<Graph> graphs = request.graphs()
                .stream()
                .map(graph -> new Graph(graph.index(), graph.sensorName(), graph.metricName(), graph.metricType(), graph.lineColor()))
                .toList();
    ...
    public GetDashboardsResponse getDashboards(UUID resourceGroupId, UUID agentId) {
        ...
        var dashboards = dashboardRepository.findAllByAgentId(agentId, Sort.by("name")).stream()
                .map(dashboard -> new GetDashboardsResponse.Dashboard(dashboard.getId(), dashboard.getName()))
                .toList();
    ...
    public GetGraphDataResponse getGraphData(UUID resourceGroupId, UUID agentId, UUID dashboardId, int graphIndex, GetGraphDataRequest request) {
        ...
        if(request.aggregationType().equals("LTTB")){
            ...
            int totalMinutes = (int) request.from().until(request.to(), java.time.temporal.ChronoUnit.MINUTES);
