...
To update the database use this command from the Web project folder (replace `ServerGuard` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../ServerGuard.Infrastructure/ServerGuard.Infrastructure.csproj -s ServerGuard.Web.csproj
```
